...
const RatingSurveys = () => {
  ...
  const handleSubmitResponse = async () => {
    ...
    if (surveyType === 'rating_text' && selectedSurvey.text_required && !responseData.text_response.trim()) {
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.post(`${API_BASE_URL}/rating-surveys/${selectedSurvey.id}/respond`, responseData, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const handleViewResponses = async (survey) => {
    try {
      const token = localStorage.getItem('token');
      const response = await axios.get(`${API_BASE_URL}/rating-surveys/${survey.id}/responses`, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const canManageSurveys = ['admin', 'super_admin', 'teacher'].includes(userRole);
    ...
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-gray-500 mt-1">Collect feedback through surveys and ratings</p>
        </div>
        {canManageSurveys && (
          ...
          <CardContent>
            <div className="space-y-2">
              {pendingSurveys.map(survey => (
                <div key={survey.id} className="flex items-center justify-between p-3 bg-white rounded-lg shadow-sm">
                  <div>
          ...
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-orange-600">{pendingSurveys.length}</p>
              </div>
      ...
      {loading ? (
        ...
        <Card>
          <CardContent className="p-12 text-center">
            ...
            <h3 className="text-lg font-medium text-gray-600">No surveys found</h3>
            <p className="text-gray-400 mt-2">
              {searchTerm ? 'Try a different search term' : 'No surveys to display'}
            </p>
          </CardContent>
        </Card>
      ) : (
              ...
              <CardContent className="p-5">
                <div className="flex items-start justify-between mb-3">
                  <div className="flex items-center space-x-2">
                    <Badge className={
                      survey.survey_type === 'rating_text' ? 'bg-purple-100 text-purple-800' :
                      survey.survey_type === 'rating_only' ? 'bg-yellow-100 text-yellow-800' :
                ...
                <div className="flex items-center justify-between text-sm text-gray-500 mb-4">
                  <span className="flex items-center">
                    ...
                    {targetRoles.find(r => r.value === survey.target_role)?.label}
                  </span>
                  <span className="flex items-center">
                ...
                {survey.average_rating > 0 && survey.survey_type !== 'mcq' && (
                  <div className="flex items-center space-x-2 mb-4">
                    {renderStars(Math.round(survey.average_rating), survey.rating_scale, null, 'sm')}
      ...
      <Dialog open={isCreateModalOpen} onOpenChange={setIsCreateModalOpen}>
        <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
            ...
            {formData.survey_type !== 'mcq' && (
              <div>
                ...
                <Select 
                  ...
                  onValueChange={(value) => setFormData({ ...formData, rating_scale: parseInt(value) })}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
              ...
              <>
                <div className="flex items-center space-x-2">
                  <input
                    ...
                    onChange={(e) => setFormData({ ...formData, text_required: e.target.checked })}
                    className="rounded border-gray-300"
                  />
                  <Label htmlFor="text_required">Text feedback is required</Label>
                </div>
                <div>
                  <Label>Text Placeholder</Label>
                  <Input
                    value={formData.text_placeholder}
                    onChange={(e) => setFormData({ ...formData, text_placeholder: e.target.value })}
          ...
          <DialogFooter>
            ...
            <Button onClick={handleCreateSurvey} className="bg-emerald-600 hover:bg-emerald-700">
              Create Survey
            </Button>
          </DialogFooter>
        </DialogContent>
      ...
      <Dialog open={isResponseModalOpen} onOpenChange={setIsResponseModalOpen}>
        <DialogContent className="max-w-md">
              ...
              {selectedSurvey.survey_type !== 'mcq' && (
                <div className="text-center">
                  ...
                  <div className="flex justify-center">
                    {renderStars(responseData.rating, selectedSurvey.rating_scale, (rating) => {
                      setResponseData(prev => ({ ...prev, rating }));
                    }, 'lg')}
                  </div>
                  {responseData.rating > 0 && (
                    <p className="mt-2 text-sm text-gray-500">
                      {selectedSurvey.rating_labels?.[responseData.rating - 1]?.label || `${responseData.rating} of ${selectedSurvey.rating_scale}`}
                    </p>
                  )}
                </div>
              )}
              ...
              {selectedSurvey.survey_type === 'rating_text' && (
                <div>
                  ...
                  <Textarea
                    ...
                    onChange={(e) => setResponseData(prev => ({ ...prev, text_response: e.target.value }))}
                    ...
                    {selectedSurvey.mcq_options?.map((option, index) => (
                      <label
                        ...
                        className={`flex items-center p-3 rounded-lg border cursor-pointer transition-colors ${
                          responseData.mcq_responses.includes(option.id || option.text)
                            ? 'border-emerald-500 bg-emerald-50'
                            : 'border-gray-200 hover:bg-gray-50'
                        }`}
                      >
                        <input
                          type={selectedSurvey.mcq_allow_multiple ? 'checkbox' : 'radio'}
                          name="mcq_response"
                          value={option.id || option.text}
                          checked={responseData.mcq_responses.includes(option.id || option.text)}
                          onChange={(e) => {
                            ...
                            if (selectedSurvey.mcq_allow_multiple) {
                              setResponseData(prev => ({
                                ...
                                mcq_responses: e.target.checked
                                  ...
                                  : prev.mcq_responses.filter(v => v !== value)
                              }));
                            } else {
                              setResponseData(prev => ({ ...prev, mcq_responses: [value] }));
                            }
                          }}
                          className="mr-3"
                        />
          ...
          <DialogFooter>
            ...
            <Button onClick={handleSubmitResponse} className="bg-emerald-600 hover:bg-emerald-700">
              <Send className="h-4 w-4 mr-2" />
              Submit Response
            </Button>
          </DialogFooter>
      ...
      <Dialog open={isViewResponsesModalOpen} onOpenChange={setIsViewResponsesModalOpen}>
        <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
          ...
          {surveyResponses && (
            <div className="space-y-6">
              <div className="grid grid-cols-3 gap-4">
                <Card>
                  <CardContent className="p-4 text-center">
                    <p className="text-2xl font-bold text-blue-600">{surveyResponses.stats.total_responses}</p>
                    <p className="text-sm text-gray-500">Total Responses</p>
                  </CardContent>
                </Card>
                {selectedSurvey?.survey_type !== 'mcq' && (
                ...
                <Card>
                  <CardContent className="p-4 text-center">
                    <p className="text-2xl font-bold text-green-600">
                      {((surveyResponses.stats.total_responses / (surveys.length || 1)) * 100).toFixed(0)}%
                    </p>
              ...
              {selectedSurvey?.survey_type !== 'mcq' && surveyResponses.stats.rating_distribution && (
                <Card>
                  <CardHeader>
                  ...
                  <CardContent>
                    <div className="space-y-2">
                      {Object.entries(surveyResponses.stats.rating_distribution).reverse().map(([rating, count]) => (
                        <div key={rating} className="flex items-center space-x-3">
                          ...
                          <div className="flex-1 bg-gray-200 rounded-full h-3">
                            <div 
                              ...
                              style={{ 
                                width: `${surveyResponses.stats.total_responses > 0 
                                  ? (count / surveyResponses.stats.total_responses) * 100 
                                  : 0}%` 
                              }}
                            />
                          </div>
                      ...
                      <div key={response.id || index} className="p-3 bg-gray-50 rounded-lg">
                        ...
                        {response.rating && (
                          <div className="flex items-center space-x-2 mb-2">
                            {renderStars(response.rating, selectedSurvey?.rating_scale || 5, null, 'sm')}
                          </div>
                        )}
                        {response.text_response && (
                          <p className="text-sm text-gray-600">{response.text_response}</p>
                        )}
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setIsViewResponsesModalOpen(false)}>
              Close
            </Button>
          </DialogFooter>
        </DialogContent>
