...
const Fees = () => {
  ...
  const generateReceipt = (receiptNo, paymentData, student) => {
    ...
    try {
      ...
      doc.text(receiptData.school.name, pageWidth / 2, 28, { align: 'center' });
      doc.setFontSize(10);
      doc.text(receiptData.school.address, pageWidth / 2, 35, { align: 'center' });
      ...
      doc.setFontSize(11);
      doc.setFont('helvetica', 'bold');
      doc.text(`Receipt No: ${receiptData.receiptNo}`, 20, yPos + 3);
      doc.text(`Date: ${receiptData.date} | Time: ${receiptData.time}`, pageWidth - 20, yPos + 3, { align: 'right' });
      ...
      drawRow('Name:', receiptData.student.name, yPos);
      yPos += 8;
      drawRow('Admission No:', receiptData.student.admissionNo, yPos);
      yPos += 8;
      drawRow('Class:', `${receiptData.student.class} - Section ${receiptData.student.section}`, yPos);
      ...
      drawRow('Fee Type:', receiptData.payment.feeType, yPos);
      yPos += 8;
      ...
      doc.text(formatCurrency(receiptData.payment.amount), 70, yPos);
      doc.setTextColor(0, 0, 0);
      doc.setFontSize(11);
      
      yPos += 8;
      drawRow('Payment Mode:', receiptData.payment.paymentMode, yPos);
      yPos += 8;
      drawRow('Transaction ID:', receiptData.payment.transactionId, yPos);
      
      if (receiptData.payment.remarks && receiptData.payment.remarks !== 'N/A') {
        yPos += 8;
        drawRow('Remarks:', receiptData.payment.remarks, yPos);
      }

      if (receiptData.payment.breakdown) {
        yPos += 15;
        doc.setFillColor(245, 245, 245);
        doc.rect(15, yPos - 5, pageWidth - 30, 30, 'F');
        doc.setFontSize(10);
        doc.setFont('helvetica', 'bold');
        doc.text('Payment Breakdown:', 20, yPos + 3);
        doc.setFont('helvetica', 'normal');
        yPos += 10;
        doc.text(`Overdue Fees: ${formatCurrency(receiptData.payment.breakdown.overduePayment)}`, 25, yPos);
        yPos += 6;
        doc.text(`Pending Fees: ${formatCurrency(receiptData.payment.breakdown.pendingPayment)}`, 25, yPos);
        if (receiptData.payment.breakdown.unappliedAmount > 0) {
          yPos += 6;
          doc.text(`Unapplied: ${formatCurrency(receiptData.payment.breakdown.unappliedAmount)}`, 25, yPos);
        }
      }
      ...
      yPos += 8;
      doc.text(`Generated on ${new Date().toLocaleString()} by School ERP System`, pageWidth / 2, yPos, { align: 'center' });

      const fileName = `Receipt-${receiptNo}-${receiptData.student.name.replace(/\s+/g, '-')}.pdf`;
      doc.save(fileName);
